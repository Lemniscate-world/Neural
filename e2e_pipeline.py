...
def load_agent():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.float16,  # nosec B615
    )
...
def agent_diagnose(events, hypotheses, chains):
    ...
    chain_text = (
        f"{chains[0].root_cause} -> {chains[0].final_symptom}" if chains else "none"
    )

    prompt = (
        f"Analyze these NeuralDBG events.\n"
        + "\n".join(event_lines)
...
def run_pipeline(bug_id, make_model_fn, category, steps=5):
    ...
    with NeuralDbg(model_h) as dbg_h:
        ...
        healthy_anomalies = len(
            [
                e
                for e in dbg_h.dump_events()
                if e["event_type"]
                in (
                    "data_anomaly",
                    "nan_detected",
    ...
    # Show diagnosis
    print(
        f"    Anomalies: {healthy_anomalies} -> {bug_anomalies} (gap: +{bug_anomalies - healthy_anomalies})"
    )
    ...
    if bug_chains:
        c = bug_chains[0]
        print(
            f"    Causal chain: {c.root_cause} -> {c.final_symptom} (conf={c.confidence:.3f}, len={c.length})"
        )
    else:
    ...
    try:
        ...
        diagnosis = agent_diagnose(
            model_llm, tokenizer, bug_events, bug_hypotheses, bug_chains
        )
        ...
        if not cat_match:
            cat_match = re.search(
                r'category.*?:\s*"?([^",}\s]+)', diagnosis, re.IGNORECASE
            )
    ...
    correct_cat = (
        predicted_cat == category
        or category in predicted_cat
        or predicted_cat in category
    )
    ...
    print(
        f"    Detected: {'YES' if detected else 'NO'} | Resolved: {'YES' if resolved else 'NO'} | Correct cat: {'YES' if correct_cat else 'NO'}"
    )
...
def make_bug006():
    ...
    bug_fn = lambda s: (
        torch.tensor([[1.0, 2.0, 3.0, 4.0, float("nan"), 6.0, 7.0, 8.0, 9.0]]),
        target,
    )
    x_fixed = torch.tensor([[1.0, 2.0, 3.0, 4.0, float("nan"), 6.0, 7.0, 8.0, 9.0]])
    x_fixed[torch.isnan(x_fixed)] = 0.0
    fix_fn = lambda s: (x_fixed, target)
    return (
        (lambda: (M(), torch.optim.SGD(M().parameters(), lr=0.001), healthy_fn)),
        (lambda: (M(), torch.optim.SGD(M().parameters(), lr=0.001), bug_fn)),
        (lambda: (M(), torch.optim.SGD(M().parameters(), lr=0.001), fix_fn)),
    )
...
for bug_id, category, make_healthy, make_bug, make_fix in [
    ...
    try:
        ...
        if b_ch:
            print(
                f"    Chain: {b_ch[0].root_cause} -> {b_ch[0].final_symptom} (conf={b_ch[0].confidence:.3f})"
            )
        else:
            print(f"    Chain: none")
        ...
        if agent_model:
            ...
            diagnosis = agent_tokenizer.decode(
...
for r in results:
    if "error" in r:
        ...
        print(
            f"  {r['bug_id']}: {r['status']} | h={r['h']} b={r['b']} f={r['f']} | pred={r['predicted']} | det={r['detected']} res={r['resolved']}"
        )
