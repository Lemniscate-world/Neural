...
def test_niveau1_installation():
    ...
    # Version
    try:
        from neuraldbg import __version__ as ver

        check(f"Version package: {ver}", bool(ver))
    except ImportError:
        pass
...
def test_niveau2_quickstart():
    ...
    try:
        ...
        check(
            f"Loss history: {len(dbg.loss_history)} steps", len(dbg.loss_history) == 5
        )

        # explain_failure sans engine
        hypotheses = dbg.explain_failure("vanishing_gradients")
        check(
            f"explain_failure retourne {len(hypotheses)} hypothèses",
            len(hypotheses) >= 0,
        )
        ...
        check(
            f"Mermaid export valide ({len(mermaid)} chars)",
            len(mermaid) > 0 and mermaid.startswith("graph TD"),
        )
        ...
        try:
            ...
            with open(path) as f:
                data = json.load(f)
            check(
                f"JSON export valide: {len(data.get('events', []))} events",
                "events" in data,
            )
        finally:
            os.unlink(path)
...
def test_niveau3_fonctionnel():
    ...
    try:
        ...
        vanish_events = [
            e
            for e in events
            if "vanishing" in str(e.to_state).lower()
            or "vanishing" in str(e.from_state).lower()
        ]
        check(
            f"Events de vanishing détectés: {len(vanish_events)}", True
        )  # Au moins le système tourne
        ...
        if hyp:
            ...
            for h in hyp:
                check(
                    f"  - {h.description[:60]}... (conf: {h.confidence:.2f})",
                    h.confidence > 0,
                )
        else:
    ...
    try:
        ...
        check(
            f"Events d'optimizer instability: {len(spike_events)}",
            len(spike_events) > 0,
        )
    except Exception as e:
