...
print(
    f"NeuralDBG: {ndbg.get('stargazers_count', 0)} stars, {ndbg.get('forks_count', 0)} forks, {ndbg.get('open_issues_count', 0)} open issues"
)
...
for num in [186786, 188053, 188066]:
    ...
    print(
        f"PR #{num}: state={pr.get('state')}, merged={pr.get('merged')}, draft={pr.get('draft')}"
    )

# 3. Workflow runs for sync-portfolio on kuro-rules
runs = get(
    "https://api.github.com/repos/Lemniscate-world/kuro-rules/actions/runs?per_page=5"
)
