...
for name, config_fn, data_bug_fn in scenarios:
    ...
    # A/B comparison: only flag if bug run exceeds healthy baseline
    detected = n_anomalies > healthy_baseline + 2
    improved = n_fixed <= healthy_baseline + 2 or n_fixed < n_anomalies
...
print(f"{'='*65}")
for r in results:
    status = "PASS" if (r["detected"] and r["improved"]) else ("OK" if not r["detected"] and r["improved"] else "FAIL")
...
print(f"\n  Auto-fix success: {passed}/{len(results)} scenarios")
