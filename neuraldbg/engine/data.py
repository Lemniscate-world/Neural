...
class DataAnalyzer:
    ...
    def check_anomaly(
        self, tensor: torch.Tensor, layer_name: str, stats_key: str | None = None
    ):
        from neuraldbg import DataHealth, EventType, SemanticEvent
        ...
        hard_anomaly = current_health in (
        ...
        if hard_anomaly:
            ...
            layer_streak
            and layer_streak[0] == current_health.value
            and layer_streak[1] == current_step - 1
        ):
            new_streak = layer_streak[2] + 1
        elif (
            layer_streak
            and layer_streak[0] == current_health.value
            and layer_streak[1] == current_step
        ):
            new_streak = layer_streak[2]
        else:
            new_streak = 1
        self.dbg._data_health_streak[layer_name] = (
                ...
                if current_health != DataHealth.NORMAL:
                    ...
                    if current_health in (
                        ...
                        if hasattr(self.dbg, "disk_cache"):
                            cache_path = self.dbg.disk_cache.save(
