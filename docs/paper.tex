...
Using NeuralDBG during development, we discovered and diagnosed six real PyTorch bugs with causal chains, including \texttt{svdvals()} silently swallowing NaN (issue \#187759), MPS gradient corruption of 100$\times$--100K$\times$ (\#177116), and \texttt{varlen\_attn()} producing silent NaN with padding (\#176793). Two upstream diagnostic test PRs were submitted to PyTorch (\#188053, \#188923). Each post-mortem includes the causal chain that NeuralDBG produced, the discovered root cause, and the lesson for training failure modes.
...
\begin{enumerate}
    ...
    \item \textbf{Upstream integration}: Two diagnostic test PRs submitted to PyTorch (\#188053, \#188923); none merged as of publication. Our process lesson, from maintainer feedback, is to discuss on the issue and secure the \emph{actionable} label before opening a PR. Long-term, a standardized training diagnostic hook API would benefit the entire ecosystem.
